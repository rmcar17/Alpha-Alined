...
StrDir TSOPController::getStrDir(){
  refresh();
  read();
  filterValues();
  errorCorrect();
...
StrDir TSOPController::getStrAng(int best_n){
  int returnAngle = 0;
  int returnStrength = 0;
  double i=0,j=0;
  for(int n=0;n<best_n;n++){
    i += (double)finalValues[n]*sin(degreesToRadians((double)finalAngles[n]));
    j += (double)finalValues[n]*cos(degreesToRadians((double)finalAngles[n]));
  }
  if(i==0){
      if(j==0){
        returnAngle = -1;
    ...
    returnAngle = 360-(int)mod(270+(int)round(radiansToDegrees(atan2(j,i))),360);
  }
